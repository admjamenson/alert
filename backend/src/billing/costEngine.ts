...
export type BillingCostProvider = 'stripe' | 'app_store' | 'play_store';
export type SupportedFixedFeeCurrency = string;

export type CommercialTerms = {
  provider: BillingCostProvider;
  percentFee: number;
  fixedFee: number;
  fixedFeeCurrency: SupportedFixedFeeCurrency;
  classification: ValueClassification;
  evidence?: string;
  percentFeeClassification?: ValueClassification;
  fixedFeeClassification?: ValueClassification;
};

export type UsageMetrics = {
  routingCalls: number | null;
...
type PriceBookLike = {
  ...
  classifications?: Record<string, PriceBookEntry>;
};

export type ClassifiedUsdAmount = {
...
export type OperationalCosts = {
  routingUnitCostUsd: number | null;
  weatherUnitCostUsd: number | null;
  platformAllocationCostUsd: number | null;
  classification: ValueClassification;
  blockers: string[];
  priceBookStatus?: string | null;
  routingUnitCostClassification?: ValueClassification;
  weatherUnitCostClassification?: ValueClassification;
  platformAllocationCostClassification?: ValueClassification;
  evidence?: {
    routingUnitCostUsd?: string;
    weatherUnitCostUsd?: string;
    platformAllocationCostUsd?: string;
  };
};

export type CostBreakdown = {
  paymentProcessorPercentCost: number | null;
  paymentProcessorPercentCostCurrency: string | null;
  paymentProcessorFixedCost: number | null;
  paymentProcessorFixedCostCurrency: SupportedFixedFeeCurrency | null;
  paymentProcessorCost: number | null;
  paymentProcessorCostCurrency: string | null;
  routingCost: number | null;
  routingCostCurrency: string | null;
  weatherCost: number | null;
  weatherCostCurrency: string | null;
  platformAllocationCost: number | null;
  platformAllocationCostCurrency: string | null;
  totalCost: number | null;
...
export type CostEngineResult = {
  ...
  fxNormalization: FxNormalizationSummary;
};
...
type StructuredClassifiedValue = ClassifiedValue & {
  currency?: string | null;
  sourcePath?: string;
  source?: string | null;
  asOf?: string | null;
...
export type FxNormalizationSummary = {
  calculationCurrency: string | null;
  classification: ValueClassification;
  blockers: string[];
  entries: FxNormalizationEntry[];
};

type NormalizedMonetaryComponent = MonetaryComponent & {
  fxNormalization?: FxNormalizationEntry | null;
};
...
const CLASSIFICATION_ORDER: ValueClassification[] = [
  'absent',
  'modeled',
  'estimated_reliable',
  'real',
...
const normalizeClassification = (
  value: unknown,
  fallback: ValueClassification = 'absent',
): ValueClassification => {
...
const weakestClassification = (
  ...classifications: Array<ValueClassification | null | undefined>
): ValueClassification => {
  const present = classifications.filter(
    (classification): classification is ValueClassification =>
...
const classifyModeledInput = (
  blockers: string[],
  classification: ValueClassification | null | undefined,
  blocker: string,
) => {
...
const readStructuredNode = (value: unknown, path: string[]): unknown => {
  let current: unknown = value;
  for (const segment of path) {
    if (!current || typeof current !== 'object') return null;
    const container = current as Record<string, unknown>;
    if (!(segment in container)) return null;
    current = container[segment];
  }
  return current;
};

const hasStructuredPriceBookPath = (
  priceBook: PriceBookLike | null | undefined,
  path: string[],
): boolean => readStructuredNode(priceBook?.document, path) !== null;

const readStructuredPriceBookValue = (
  priceBook: PriceBookLike | null | undefined,
  path: string[],
): StructuredClassifiedValue | null => {
  const rawNode = readStructuredNode(priceBook?.document, path);
  if (!rawNode || typeof rawNode !== 'object') return null;

  const entry = rawNode as StructuredPriceBookEntry;
  const classification = normalizeClassification(
    entry.origin || entry.classification,
    'absent',
  );
  const value = normalizeAmount(
    entry.value ?? entry.amountUsd ?? entry.costUsd ?? entry.rate,
  );
  ...
  return {
    ...
    classification,
    currency: toCurrencyCode(entry.currency),
    sourcePath,
    source:
      typeof entry.source === 'string'
        ? String(entry.source).trim() || null
        : null,
    asOf: normalizeDateLike(entry.asOf || entry.effectiveDate),
    evidence: joinEvidence(
      typeof entry.evidence === 'string' ? entry.evidence : null,
      typeof entry.source === 'string'
        ? `pricebook_entry_source=${entry.source}`
        : null,
      typeof entry.sourceKey === 'string'
        ? `pricebook_source_key=${entry.sourceKey}`
        : null,
      normalizeDateLike(entry.asOf || entry.effectiveDate)
        ? `pricebook_as_of=${normalizeDateLike(entry.asOf || entry.effectiveDate)}`
        : null,
...
const buildLegacyCurrencyNormalizationKey = (
  fromCurrency: string,
  toCurrency: string,
): string => {
...
const readFirstStructuredPriceBookValue = (
  priceBook: PriceBookLike | null | undefined,
  paths: string[][],
): StructuredClassifiedValue | null => {
...
export const buildUsageMetrics = ({
  routingCalls,
  weatherCalls,
  classification = 'absent',
}: {
  routingCalls: number | null | undefined;
  weatherCalls: number | null | undefined;
  classification?: ValueClassification;
}): UsageMetrics => {
...
export const buildCommercialTermsFromPriceBook = ({
  provider,
  priceBook,
  revenueCurrency,
}: {
  provider: BillingCostProvider | null;
  priceBook: PriceBookLike | null | undefined;
  ...
  if (provider === 'stripe') {
    ...
    const fixedStructured = readStructuredPriceBookValue(priceBook, [
      'commercialTerms',
      'stripe',
      'fixedFee',
    ]);
    const fixed: StructuredClassifiedValue = fixedStructured || {
    ...
    return {
      ...
      fixedFeeCurrency:
        (fixed.currency as SupportedFixedFeeCurrency | null) || 'USD',
...
export const buildOperationalCostsFromPriceBook = ({
  priceBook,
  ...
  priceBook: PriceBookLike | null | undefined;
  platformAllocationCostUsd?: ClassifiedUsdAmount | null;
  routingUnitCostUsd?: ClassifiedUsdAmount | null;
  weatherUnitCostUsd?: ClassifiedUsdAmount | null;
}): OperationalCosts => {
...
const calculateUsageCost = ({
  ...
  count: number | null;
  usageClassification: ValueClassification;
  unitCost: number | null;
  unitCostClassification: ValueClassification | null | undefined;
  missingUsageBlocker: string;
  missingUnitCostBlocker: string;
  modeledUnitCostBlocker: string;
}): MonetaryComponent => {
  const blockers: string[] = [];
...
const buildPaymentPercentComponent = (
  revenue: RevenueSnapshot,
  commercialTerms: CommercialTerms,
): NormalizedMonetaryComponent => {
  const blockers: string[] = [];
...
const buildPaymentFixedComponent = (
  revenue: RevenueSnapshot,
  commercialTerms: CommercialTerms,
): NormalizedMonetaryComponent => {
  const blockers: string[] = [];
...
const normalizeMoneyToCurrency = ({
  ...
  priceBook: PriceBookLike | null | undefined;
  appliedTo: string;
  missingRateBlocker?: string;
  modeledRateBlocker?: string;
}): NormalizedMonetaryComponent => {
...
const combineMonetaryComponents = (
  components: MonetaryComponent[],
  modeledBlockers: string[] = [],
): MonetaryComponent => {
...
const summarizeFxNormalization = ({
  calculationCurrency,
  components,
}: {
  calculationCurrency: string | null;
  components: Array<NormalizedMonetaryComponent | MonetaryComponent>;
}): FxNormalizationSummary => {
  const entries = components
    .map(component =>
      'fxNormalization' in component ? component.fxNormalization || null : null,
    )
    .filter((entry): entry is FxNormalizationEntry => Boolean(entry));
  ...
  const mergedEntries = Array.from(
    entries.reduce((map, entry) => {
      ...
      return map;
    }, new Map<string, FxNormalizationEntry>()),
  )
    .map(([, entry]) => entry)
    .sort((left, right) =>
...
export const analyzeCostAndMargin = ({
  revenue,
  commercialTerms,
  operationalCosts,
  usageMetrics,
  ...
  const platformNative: MonetaryComponent = {
    amount:
  ...
  let marginAnalysis: MarginAnalysis = {
    totalCost: null,
    margin: null,
    marginPercent: null,
    classification: 'absent',
