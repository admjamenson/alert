...
const buildRelaySosLogEntry = (event, payload = {}) => {
  const entry = {
    timestamp: nowIso(),
    event: String(event || '').trim() || 'relay_sos_unknown',
    requestId: sanitizeId(payload.requestId),
    relayId: sanitizeId(payload.relayId),
...
const logRelaySosEvent = (logger, event, payload = {}) => {
  const entry = buildRelaySosLogEntry(event, payload);
  const line = JSON.stringify(entry);
  const target = logger || console;

  if (event === 'relay_sos_request_failed') {
...
const buildRelaySosRequestId = value => sanitizeId(value) || crypto.randomUUID();
