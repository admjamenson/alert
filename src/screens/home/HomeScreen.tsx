...
import { PermissionManager, type PermissionStatus } from '../../utils/permissions';
import {
...
const getWeatherWidget = () => require('../../components/home/WeatherWidget').WeatherWidget;
const getRiskMapWidget = () => require('../../components/home/RiskMapWidget').RiskMapWidget;
...
const isWeatherThreatCategory = (value?: string) =>
  WEATHER_THREAT_CATEGORIES.has(String(value || '').trim().toLowerCase());
...
const guessRealtimeCategoryFromAlert = (title: string): string | undefined => {
  ...
  if (t.includes('epidem') || t.includes('influenza') || t.includes('gripe')) return 'epidemic';
  if (t.includes('energia') || t.includes('power')) return 'energy_outage';
  if (t.includes('agua') || t.includes('água') || t.includes('water')) return 'water_outage';
  if (t.includes('terremoto') || t.includes('earthquake')) return 'earthquake';
  if (t.includes('inunda') || t.includes('alag') || t.includes('flood')) return 'flood';
  if (t.includes('tempest') || t.includes('storm')) return 'storm';
  if (t.includes('raio') || t.includes('lightning')) return 'lightning';
  if (t.includes('vento') || t.includes('vendaval') || t.includes('wind')) return 'wind';
  if (t.includes('sos')) return 'storm';
  return undefined;
};

const inferWeatherThreatCategory = (...values: Array<string | undefined>): string | undefined => {
  for (const rawValue of values) {
    const directCategory = guessRealtimeCategoryFromAlert(String(rawValue || ''));
...
const HomeScreen: React.FC = ({ navigation }: any) => {
  ...
  const { securityState, triggerSecureSOS, setPanicHold, requestPreciseFixNow } = useSecurity();
  const insets = useSafeAreaInsets();
  const [refreshing, setRefreshing] = useState(false);
  const [alerts, setAlerts] = useState<AlertNotification[]>([]);
  const [prioritizedRisks, setPrioritizedRisks] = useState<RankedRisk[]>([]);
  const [hasUnavailableMonitoringData, setHasUnavailableMonitoringData] = useState(false);
  const [operationalModel, setOperationalModel] = useState<OperationalSnapshotReadModel>({
    snapshot: null,
    state: 'loading',
    stale: true,
    errorCode: 'initial',
  });
  const [brainBriefing, setBrainBriefing] = useState<AlertBrainBriefingReadModel>(
    createInitialAlertBrainBriefingReadModel(),
  );
  ...
  const safetyCTAState = useMemo(
    () => {
      ...
      if (operationalRisk === 'low' && operationalModel.state !== 'error') return 'ok';
      return getSafetyStateFromRisks(prioritizedRisks);
    },
    [operationalModel.snapshot?.riskLevel, operationalModel.state, prioritizedRisks],
  );
  ...
  const topRisk = useMemo(() => prioritizedRisks[0] || null, [prioritizedRisks]);
  ...
  const barAccessibilityLabel = useMemo(() => {
    const riskLabel = hasUnavailableMonitoringData && statusBarState === 'ok'
    ...
    const freshnessLabel = operationalModel.snapshot
      ? t('operational_freshness_minutes', {
          count: Math.max(1, Math.round(Number(operationalModel.snapshot.freshnessSec || 0) / 60)),
  ...
  const refreshLocationPermissionStatus = useCallback(async (): Promise<PermissionStatus> => {
    try {
  ...
  const handleSOS = async () => {
    ...
    try {
      ...
      TelemetryService.trackEvent('sos_send_error', { screen: 'home' });
      Alert.alert(
        t('sos_failed_title'),
        t('sos_failed_body'),
      );
  ...
  const handleOpenMessages = useCallback(() => {
    ...
    navigation.navigate('Conversations', { openGuardiansFirst: false });
    void (async () => {
      try {
  ...
  const fetchAlerts = useCallback(
    async (force = false, requestToken?: number) => {
      ...
      const [
        ...
        MonitoringService.getActiveEvents(safeLat, safeLon, riskScore).catch(() => null),
        NotificationService.getActiveSos(),
        RouteDestinationService.getDefaultDestination(),
        EpidemicService.getSnapshot(safeLat, safeLon, 'pandemic', '7d').catch(() => null),
        EpidemicService.getSnapshot(safeLat, safeLon, 'epidemic', '7d').catch(() => null),
      ...
      const topWeatherRisk = ranked.find(risk => isWeatherThreatCategory(risk.categoryId)) || null;
      ...
      const weatherImportantAlertCandidate =
        hasWeatherEvidence && inferredWeatherCategory
          ? buildWeatherImportantAlertCandidateFromSignal({
              ...
              label: t(getWeatherImportantAlertLabelKey(inferredWeatherCategory)),
              latitude: safeLat,
              longitude: safeLon,
              localeTag,
  ...
  }, [fastStartReady, navigation, fetchAlerts, refreshLocationPermissionStatus]);
  ...
  useEffect(() => {
    ...
    const measure = performance.measure('home_map_ready', 'app_ready', 'home_map_ready');
    if (__DEV__ && measure?.duration) {
      console.log(`[ALERT-PERF] home_map_ready=${Math.round(measure.duration)}ms`);
    }
  }, []);

  useEffect(() => {
    performance.mark('home_render');
    const measure = performance.measure('home_render', 'app_ready', 'home_render');
    if (__DEV__ && measure?.duration) {
        ...
        >
              ...
              <View style={styles.mapPlaceholderWrap}>
                ...
                >
                  <View style={styles.mapPlaceholderBody}>
                    <Icon name="map-outline" size={24} color={colors.textSecondary} />
                    <Text
                      style={[
          ...
          >
            <View style={styles.sosWrap}>
              ...
              >
                <Text style={[styles.sosText, { fontFamily: FONT_FAMILY }]}>
                  {t('sos')}
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <View
              style={[
                styles.alertBar,
                { backgroundColor: barVisual.backgroundColor, borderColor: barBorderColor },
              ]}
            >
              <View
                pointerEvents="none"
        ...
        <View
          ...
          pointerEvents="box-none"
            >
              ...
              >
              <Icon name="message-text-outline" size={20} color={messagesBarVisual.iconColor} />
              <Text style={[styles.messagesBarText, { color: messagesBarVisual.textColor }]}>
                {t('messages_title')}
              </Text>
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  sosButton: {
    ...
    borderWidth: 6,
    borderColor: 'rgba(255, 255, 255, 0.2)', // Detalhe de borda interna
  },
  sosText: {
    color: '#FFF',
    fontSize: 28,
